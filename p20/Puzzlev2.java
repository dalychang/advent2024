...
public class Puzzlev2 {
    ...
    while (!paths.isEmpty()) {
      ...
      for (Direction d : Direction.values()) {
        ...
        if (nextPosition.equals(end)) {
          ...
          scoreMap.put(nextPosition, bestScore);
          continue;
        }
  ...
  public static void cheatTraverse(char[][] grid, int maxX, int maxY, Map<Position, Long> scoreMap, Map<Cheat, Long> cheatDeltaMap, Position cheatStart, int steps) {
    for (int y = 0; y < maxY; y++) {
      for (int x = 0; x < maxX; x++) {
        int distance = Math.abs(cheatStart.x - x) + Math.abs(cheatStart.y - y);
        if (distance > steps) {
          continue;
        }
        ...
        Position cheatEnd = new Position(x, y);
        if (!scoreMap.containsKey(cheatEnd)) {
          continue;
        }

        long scoreEnd = scoreMap.get(cheatEnd);
        long scoreStart = scoreMap.get(cheatStart);
        long delta = scoreEnd - scoreStart - distance;
        Cheat cheat = new Cheat(cheatStart, cheatEnd);
        cheatDeltaMap.put(cheat, delta);
      }
    }
  }
  ...
  public static void calculateCheats(char[][] grid, int maxX, int maxY, Position start, Position end, Map<Position, Long> scoreMap, Map<Cheat, Long> cheatDeltaMap) {
    for (int y = 0; y < maxY; y++) {
      for (int x = 0; x < maxX; x++) {
        ...
        cheatTraverse(grid, maxX, maxY, scoreMap, cheatDeltaMap, cheatStart, 20);
      }
    }
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    long bestScore = traverse(grid, maxX, maxY, start, end, scoreMap);
