...
// This one solves it with a long of manual work.
public class Puzzlev4 {
  ...
  private static final long INCREMENTS = 0100000000000000L;
  ...
  private static final List<Long> FIRST_DIGITS = List.of(072L, 073L, 076L, 077L);
  private static final List<Long> SECOND_DIGITS = List.of(042L, 047L);
  private static final List<Long> THIRD_DIGITS = List.of(051L);
  private static final List<Long> FOURTH_DIGITS = List.of(056L, 057L);
  private static final List<Long> FIFTH_DIGITS = List.of(043L, 003L);
  private static final List<Long> SIXTH_DIGITS = List.of(034L, 042L);
  private static final List<Long> SEVENTH_DIGITS = List.of(026L);
  ...
  public static void main(String[] args) throws Exception {
    ...
    long answer = 0;
    while (!done) {
      long testValueStart = currentIncrement * INCREMENTS;
      for (int z = 0; z < FIRST_DIGITS.size(); z++) {
        for (int y = 0; y < SECOND_DIGITS.size(); y++) {
          for (int x = 0; x < THIRD_DIGITS.size(); x++) {
            for (int w = 0; w < FOURTH_DIGITS.size(); w++) {
              for (int v = 0; v < FIFTH_DIGITS.size(); v++) {
                for (int u = 0; u < SIXTH_DIGITS.size(); u++) {
                  for (int t = 0; t < SEVENTH_DIGITS.size(); t++) {
                
                    long testValue = SEVENTH_DIGITS.get(t);
                    testValue = (testValue << 6) + SIXTH_DIGITS.get(u);
                    testValue = (testValue << 6) + FIFTH_DIGITS.get(v);
                    testValue = (testValue << 6) + FOURTH_DIGITS.get(w);
                    testValue = (testValue << 6) + THIRD_DIGITS.get(x);
                    testValue = (testValue << 6) + SECOND_DIGITS.get(y);
                    testValue = (testValue << 6) + FIRST_DIGITS.get(z);
                    testValue = testValue + testValueStart;
